...
claude --dangerously-skip-permissions --print \"你是项目经理Agent。项目目标：${PROJECT_GOAL}\n\n请输出：1.任务清单(JSON:task_id,title,owner_agent,priority,estimated_days,depends_on) 2.关键里程碑(M1-M5对应5个阶段) 3.风险预判(每项附对应部门Agent)\" | tee /tmp/agentflow/task_queue.json
echo 'DONE:PM' >> /tmp/agentflow/task_queue.json
echo '=== PM Agent 完成 ==='
" Enter
...
claude --dangerously-skip-permissions --print \"你是工业设计师Agent。读取任务队列：\$(cat /tmp/agentflow/task_queue.json)\n\n请完成：1.产品外观设计方案(2D/3D渲染图描述) 2.材料选择建议(CMF:Color/Material/Finish) 3.色彩定义与配色方案 4.人机交互外观设计\" | tee /tmp/agentflow/id_output.json
echo 'DONE:ID' >> /tmp/agentflow/id_output.json
echo '=== ID Agent 完成 ==='
" Enter
...
claude --dangerously-skip-permissions --print \"你是结构工程经理Agent。读取外观方案：\$(cat /tmp/agentflow/id_output.json)\n\n请完成：1.主体结构设计方案 2.内部堆叠与空间规划 3.防水/防尘/散热等工程方案 4.结构件BOM初稿\" | tee /tmp/agentflow/me_manager_output.json
echo 'DONE:ME_MANAGER' >> /tmp/agentflow/me_manager_output.json
...
claude --dangerously-skip-permissions --print \"你是结构工程主管Agent。读取结构方案：\$(cat /tmp/agentflow/me_manager_output.json)\n\n请完成：1.3D结构图定义(部件关系、公差分析) 2.装配方案设计(SOP草案) 3.模具需求评估 4.与ID方案的对齐检查\" | tee /tmp/agentflow/me_lead_output.json
echo 'DONE:ME_LEAD' >> /tmp/agentflow/me_lead_output.json
...
claude --dangerously-skip-permissions --print \"你是系统架构师Agent。读取：任务队列\$(cat /tmp/agentflow/task_queue.json)\n3D结构与装配方案：\$(cat /tmp/agentflow/me_lead_output.json)\n\n请完成：1.系统功能架构设计(模块划分、数据流、控制流) 2.软硬件接口约定 3.通信协议定义(BLE/WiFi/UART/I2C/SPI) 4.数据存储与同步方案 5.安全与加密方案\" | tee /tmp/agentflow/architect_output.json
echo 'DONE:ARCHITECT' >> /tmp/agentflow/architect_output.json
...
claude --dangerously-skip-permissions --print \"你是硬件工程师Agent。读取架构方案：\$(cat /tmp/agentflow/architect_output.json)\n\n请完成：1.硬件选型方案与BOM清单(料号/规格/供应商) 2.PCB需求描述(层数/尺寸/阻抗要求) 3.固件版本号定义(v{major}.{minor}.{patch}) 4.接口约定对齐(I2C/SPI/UART协议参数) 5.测试点定义\" | tee /tmp/agentflow/hw_output.json
echo 'DONE:HW_LEAD' >> /tmp/agentflow/hw_output.json
...
claude --dangerously-skip-permissions --print \"你是PCB应用工程师Agent。读取：BOM与PCB需求\$(cat /tmp/agentflow/hw_output.json)\n架构接口约定：\$(cat /tmp/agentflow/architect_output.json)\n\n请完成：1.PCB布局方案(关键器件placement) 2.信号完整性分析 3.电源完整性分析 4.EMC/EMI设计考虑 5.Gerber文件需求描述\" | tee /tmp/agentflow/hw_board_output.json
echo 'DONE:HW_BOARD' >> /tmp/agentflow/hw_board_output.json
...
claude --dangerously-skip-permissions --print \"你是固件工程师Agent。读取：架构接口约定\$(cat /tmp/agentflow/architect_output.json)\nBOM与固件版本：\$(cat /tmp/agentflow/hw_output.json)\n\n请完成：1.固件架构设计(RTLS/Bare-metal选择、任务划分) 2.外设驱动代码骨架(I2C/SPI/UART/GPIO/ADC) 3.通信协议栈实现(BLE/WiFi/MQTT) 4.OTA升级方案 5.功耗优化方案 6.单元测试框架\" | tee /tmp/agentflow/fw_output.json
echo 'DONE:FW' >> /tmp/agentflow/fw_output.json
...
claude --dangerously-skip-permissions --print \"你是App开发工程师Agent。读取架构方案：\$(cat /tmp/agentflow/architect_output.json)\n\n请完成：1.App技术选型(iOS/Android/Flutter/React Native) 2.UI组件设计与状态管理方案 3.API调用层实现 4.设备连接与通信模块(BLE/WiFi) 5.数据持久化方案 6.单元测试框架\" | tee /tmp/agentflow/app_output.json
echo 'DONE:APP' >> /tmp/agentflow/app_output.json
...
claude --dangerously-skip-permissions --print \"你是后端工程师Agent。读取架构方案：\$(cat /tmp/agentflow/architect_output.json)\n\n请完成：1.后端技术栈选择与API服务设计(REST/GraphQL) 2.数据库Schema设计 3.用户认证与授权方案 4.设备管理API(注册/状态/OTA推送) 5.数据上报与统计分析API 6.部署方案与监控告警\" | tee /tmp/agentflow/backend_output.json
echo 'DONE:BACKEND' >> /tmp/agentflow/backend_output.json
...
echo 'DONE:VERIFICATION' >> /tmp/agentflow/verification_output.json
